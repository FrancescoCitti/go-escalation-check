...
var Techniques = []Technique{
	...
	},
	{
		...
		Name:        "Directly Assume Admin Role",
		Description: "Call sts:AssumeRole on a role with administrator access whose trust policy already permits the caller",
		Required:    []string{"sts:AssumeRole"},
		MitreKey:    "assume_role",
		Severity:    "high",
	},
	{
		ID:          "deactivate_mfa_device",
		Name:        "Deactivate MFA Device on Admin User",
		Description: "Remove MFA from a user with administrator privileges, enabling password-only console access",
		Required:    []string{"iam:DeactivateMFADevice"},
		MitreKey:    "mfa_removal",
		Severity:    "critical",
	},
	{
		ID:          "delete_virtual_mfa",
		Name:        "Delete Virtual MFA Device from Admin User",
		Description: "Delete the virtual MFA device from a user with administrator privileges",
		Required:    []string{"iam:DeleteVirtualMFADevice"},
		MitreKey:    "mfa_removal",
		Severity:    "critical",
	},
	{
		ID:          "update_access_key",
		Name:        "Reactivate Disabled Access Key for Admin User",
		Description: "Re-enable a deactivated programmatic credential belonging to a user with administrator privileges",
		Required:    []string{"iam:UpdateAccessKey"},
		MitreKey:    "update_access_key",
		Severity:    "high",
	},
	{
		ID:          "pass_role_sagemaker",
		Name:        "Pass Admin Role via SageMaker",
		Description: "Create a SageMaker notebook instance with an admin role to execute arbitrary code",
		Required:    []string{"iam:PassRole", "sagemaker:CreateNotebookInstance"},
		MitreKey:    "pass_role",
		Severity:    "high",
	},
	{
		ID:          "pass_role_codebuild",
		Name:        "Pass Admin Role via CodeBuild",
		Description: "Create and start a CodeBuild project with an admin role to execute arbitrary build commands",
		Required:    []string{"iam:PassRole", "codebuild:CreateProject", "codebuild:StartBuild"},
		MitreKey:    "pass_role",
		Severity:    "high",
	},
	{
		ID:          "pass_role_ecs",
		Name:        "Pass Admin Role via ECS",
		Description: "Register an ECS task definition and run it with an admin role attached",
		Required:    []string{"iam:PassRole", "ecs:RegisterTaskDefinition", "ecs:RunTask"},
		MitreKey:    "pass_role",
		Severity:    "high",
	},
	{
		ID:          "pass_role_datapipeline",
		Name:        "Pass Admin Role via Data Pipeline",
		Description: "Create a Data Pipeline with an admin role to execute commands on provisioned resources",
		Required:    []string{"iam:PassRole", "datapipeline:CreatePipeline", "datapipeline:PutPipelineDefinition"},
		MitreKey:    "pass_role",
		Severity:    "medium",
	},
}
